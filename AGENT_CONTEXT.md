...
| Data | Alteração |
|------|-----------| 
| 2026-06-11 | **Seletor Dinâmico de Linhas no Detalhamento (v34)**: Adicionado um slider (`slider_top_n_maq`) na tela de controle do A3 para permitir ajustar dinamicamente de 1 a 5 a quantidade de motivos principais listados por máquina na tabela do Detalhamento (Seção 8), possibilitando o alinhamento de altura das colunas conforme o volume de dados e o layout de impressão. |
| 2026-06-11 | **Fusão Lean e Detalhamento de Paradas no A3 (v33)**: Mescladas as seções 9 (Análise Lean de Severidade) e 8 (Detalhamento de Paradas por Máquina e Motivo) em uma única visualização de Detalhamento de Paradas (mantendo o número de seção como 8), incorporando distintivos coloridos (badges) baseados nas cores semafóricas do Farol Lean (verde para Operacional, amarelo para Intervenção, vermelho para Crítica) diretamente ao lado de cada motivo. Removidos os cartões de severidade da coluna direita para liberar considerável espaço vertical e reordenada a seção inferior de Ocorrências Críticas para o número 9. |
| 2026-06-11 | **Remoção de Rádio Redundante e Chave de Custos no A3 (v32)**: (1) Removido o seletor radio button `Unidade do Gráfico de Motivos` (que chaveava entre Minutos e Horas), fixando de forma definitiva todos os cálculos, labels e eixos de gráficos de inatividade para a unidade de Horas. (2) Mantido o toggle switch `Analisar Custos Financeiros` (desativado por padrão) para ocultar as colunas de taxas, custos e prejuízos no Relatório A3, e corrigida a ordenação de tabelas para manter o Pareto e o Detalhamento de paradas por máquina (Cimef, Barsanti, Resina, etc.) independentes e sequenciais. |
| 2026-06-10 | **Exibição do ID Previsto no Lançamento (v29)**: Adicionada a visualização do ID do apontamento em tempo real para o usuário durante o fluxo de lançamento manual (Aba Lançar Apontamento). O ID previsto (calculado somando o próximo ID disponível no Excel com a quantidade de itens presentes no carrinho) agora é exibido: (1) Diretamente no cabeçalho do formulário, (2) Na notificação temporária (`st.toast`) ao adicionar o item ao carrinho, (3) Em uma nova coluna na tabela de visualização do carrinho e (4) Na mensagem de sucesso após salvar o lote final no Excel. |
| 2026-06-10 | **Correção no Salvamento de Paradas Manuais (v28)**: Corrigido o bug onde o sistema não salvava os apontamentos de paradas da serraria e outras máquinas durante o lançamento manual (Aba Lançar Apontamento). A função `add_paradas` no backend (`data_manager.py`) foi corrigida para resolver chaves lógicas (como `DIA_INICIO`, `DIA_FIM`) usando o mapeamento centralizado de aliases (`_get_system_mapping()`), assim como já era feito no salvamento em lote e na edição de apontamentos. |
| 2026-06-10 | **Prevenção de Travamento por Trava de Rede e Aba 'None' (v27)**: Corrigido o bug crítico onde, após a edição de um apontamento, a aplicação perdia a referência das abas do Excel (mostrando erro 'None'). Implementadas três defesas principais no backend: (1) Unificação das chamadas de escrita de edição em `dm.update_apontamento_full`, carregando e salvando a planilha apenas uma vez em vez de duas consecutivas; (2) Criação de funções de leitura e escrita com retentativas automáticas e backoff exponencial (`_read_excel_with_retry`, etc.) para absorver latências e travas temporárias do drive de rede mapped `Z:`; (3) Fallback robusto na função `_get_sheet` para garantir que se uma aba for configurada como `None` ou vazia em memória (devido a selectboxes temporários vazios), retorne imediatamente a aba padrão (ex: `"DB"`). |
...
6. **File Locks on Mapped Network Drives (Z:)**: O sistema roda sob um drive de rede mapeado (`Z:`), onde a latência de I/O e a persistência de travas oportunistas são elevadas. Operações consecutivas de escrita (ex: salvar cabeçalhos e depois relações) geram `PermissionError` e corrompem leituras subsequentes. Sempre unifique escritas sob um único carregamento/salvamento do workbook no Excel (`dm.update_apontamento_full`) e utilize funções com retentativas automáticas e backoff exponencial (`_read_excel_with_retry`, `_load_workbook_with_retry`, etc.).
7. **Abas 'None' em Configurações por Erro de Selectbox**: Em caso de falha de carregamento do Excel (como arquivos temporariamente travados), a leitura de nomes de abas retornará `[]`. Isso faz com que selectboxes vinculados a essas abas na UI devolvam `None`. Implementar fallback estrito no `_get_sheet` (se o valor for `None`, `"None"` ou vazio, retorna o nome padrão da constante) evita que o sistema grave `null` nas configurações e perca permanentemente as referências.
